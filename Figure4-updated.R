...
df_wide <- read_excel("/media/4tb2/Daniela/Biota/PipelineBiota/paraPaper/Scripts Reproducir Paper/Fig4-RA6genera.xlsx")
...
plots <- df_final %>%
  ...
  map(~ ggplot(.x, aes(x = Mean, y = Difference, color = Comparison, shape = Comparison)) +
        ...
        geom_hline(aes(yintercept = Diff_mean, color = Comparison), linetype = "solid", linewidth = 0.4) +
        ...
        #geom_hline(aes(yintercept = Diff_mean, color = Comparison, linetype = Comparison), linewidth = 0.4) +
        geom_hline(aes(yintercept = Upper_limit, color = Comparison, linetype = Comparison), linewidth = 0.4) +
        geom_hline(aes(yintercept = Lower_limit, color = Comparison, linetype = Comparison), linewidth = 0.4) +
...
resumen_desvios <- df_final %>%
  group_by(Genus, Comparison) %>%
  summarise(
    SD = min(Diff_SD, na.rm = TRUE),
    .groups = "drop"
  )
resumen_desvios[,3:ncol(resumen_desvios)] <- round(resumen_desvios[,3:ncol(resumen_desvios)], 2)
