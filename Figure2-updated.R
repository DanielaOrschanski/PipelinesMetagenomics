...
wilcox.test(df_wide$Bowtie, df_wide$DRAGEN, paired = TRUE)

wilcox_results <- map(method_pairs, function(pair) {
  test <- wilcox.test(df_wide[[pair[1]]], df_wide[[pair[2]]], paired = TRUE)
  tibble(
    Method1 = pair[1],
    Method2 = pair[2],
...
library(dplyr)
dragen_df <- MappedReads_AllMethods %>%
  filter(Method == "DRAGEN") %>%
  rename(MappedReads_DRAGEN = MappedReads) %>%
  select(Sample, MappedReads_DRAGEN)

comparisons_df <- MappedReads_AllMethods %>%
  filter(Method != "DRAGEN") %>%
  left_join(dragen_df, by = "Sample") %>%
  mutate(Difference = MappedReads - MappedReads_DRAGEN)

# Usamos ggplot para hacer un boxplot de la diferencia
library(ggplot2)
ggplot(comparisons_df, aes(x = Method, y = Difference, fill = Method)) +
  geom_violin(trim = FALSE, alpha = 0.6, color = NA) +
  geom_jitter(aes(color = Method), width = 0.2, size = 1.2, alpha = 0.7) +
  geom_boxplot(width = 0.6, outlier.shape = NA, color = "black", alpha = 0.8) +
  scale_fill_manual(values = c(
  ...
  labs(title = "",
       x = "Compared Method",
       y = "Difference in % Mapped Reads")+ theme(legend.position = "none")
